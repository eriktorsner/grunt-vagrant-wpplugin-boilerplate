...
if [[  $ping_result == "Connected" ]]; then
	...
	if [[ ! -d /srv/www/wordpress-default ]]; then
		...
		mkdir -p /srv/www/wordpress-default
		cd /srv/www/wordpress-default
		wp core download --force
		echo "Configuring WordPress Stable..."
		wp core config --dbname=wordpress --dbuser=wordpress --dbpass=wordpress --quiet --extra-php <<PHP
define( 'WP_DEBUG', true );
PHP
		URL=www.$HOSTNAME.local
		echo "Installing WordPress Stable..."
		wp core install --url=$URL --quiet --title="Local WordPress Dev" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
	else
